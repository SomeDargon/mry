...
set server_host=115.28.173.39
set username=root
set password=uyi2July18

set http_port=8080

sed -i "s/pic.server.host=http:\/\/localhost:8080/pic.server.host=http:\/\/%server_host%/g" web\src\main\resources\config.properties

echo.&echo ======= Maven Package =================
call mvn clean install
echo.&echo ======= Maven Package End =============
sed -i "s/pic.server.host=http:\/\/%server_host%/pic.server.host=http:\/\/localhost:8080/g" web\src\main\resources\config.properties
rm -f sed*
